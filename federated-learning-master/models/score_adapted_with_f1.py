...
def calculate_acc(model):
    ...
    # 计算F1分数
    return accuracy_score(all_labels, all_predictions)  # 计算acc分数
...
def True_Shapley_f1(args, w_locals, idxs_users, w_glob, grads_locals, grad_glob_new, grad_glob):
    """基于F1评分的True Shapley实现"""
    ...
    for perm in permutations:
        ...
        for i in range(num_active_users):
            ...
            if contribution >= 0:
                ...
                # 截断负值（contribution不计）
                shapley_values[user_idx] = (t - 1) / t * shapley_values[user_idx]

    # 标准化处理
    shapley_values /= np.sum(shapley_values)
    return shapley_values

def True_Shapley_acc(args, w_locals, idxs_users, w_glob, grads_locals, grad_glob_new, grad_glob):
    """修正后的acc True Shapley实现"""
    num_total_users = args.num_users
    shapley_values = np.zeros(num_total_users)
    num_active_users = len(idxs_users)
    ...
    for perm in permutations:
        t += 1
        current_w = copy.deepcopy(w_glob)
        acc_previous = calculate_acc(current_w)
        ...
        for i in range(num_active_users):
            ...
            # 计算贡献
            acc_current = calculate_acc(w_subset)
            contribution = acc_current - acc_previous
    ...
    epsilon = 1e-10
    if abs(sum_shapley) < epsilon:
        # 处理分母接近 0 的情况
        shapley_values = np.ones_like(shapley_values) / len(shapley_values)
    else:
        shapley_values /= (sum_shapley + epsilon)  # 添加极小值保护
    return shapley_values
