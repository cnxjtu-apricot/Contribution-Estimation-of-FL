...
class LocalUpdate_partial_prisoned(object):
    def __init__(self, args, idx_user, dataset=None, idxs=None, train=True):
        ...
        if train:
            if idx_user <= int(args.num_users * args.poisoned_rate):
                self.ldr_train = DataLoader(Poisoned_DatasetSplit(dataset, idxs), batch_size=self.args.local_bs, shuffle=True)
            else:
                self.ldr_train = DataLoader(DatasetSplit(dataset, idxs), batch_size=self.args.local_bs, shuffle=True)
            self.ldr_test = None
